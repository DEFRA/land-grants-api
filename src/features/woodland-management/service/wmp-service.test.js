...
describe('validateWoodlandManagementPlan', () => {
    ...
    it('should calculate total area correctly and successfully validate woodland management plan', async () => {
        splitParcelId.mockImplementation((id) => ({ sheetId: 'sheet', parcelId: id }))
        ...
        expect(getLandData).toHaveBeenCalledWith('sheet', 'parcel1', mockRequest.server.postgresDb, mockRequest.logger)
        expect(getLandData).toHaveBeenCalledWith('sheet', 'parcel2', mockRequest.server.postgresDb, mockRequest.logger)

        expect(getEnabledActions).toHaveBeenCalledWith(mockRequest.logger, mockRequest.server.postgresDb)
    ...
    it('should ignore parcels with no land data when calculating total area', async () => {
        splitParcelId.mockImplementation((id) => ({ sheetId: 'sheet', parcelId: id }))
        getLandData.mockResolvedValueOnce(null).mockResolvedValueOnce([{ area: 50 }])
        getEnabledActions.mockResolvedValue([{ code: 'PA3', rules: ['ruleA'] }])
        executeRules.mockReturnValue({ passed: true, results: [] })
        wmpResultTransformer.mockReturnValue({ hasPassed: true })
