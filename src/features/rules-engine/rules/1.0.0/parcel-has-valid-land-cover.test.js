...
describe('parcelHasValidLandCover', () => {
  ...
  test('should fail when parcel does not have a matching land cover class code', () => {
    ...
    expect(result.reason).toEqual(defaultFailureMessage)
  })

  test('should pass when the parcel land cover class code matches the action land cover code', () => {
    const result = parcelHasValidLandCover.execute(
      createApplication(
        [{ landCoverCode: '111' }],
        [{ landCoverClassCode: '111', areaSqm: 100 }]
      ),
      createRule()
    )
  ...
  test('should pass when the action land cover matches on either class code or cover code', () => {
    const result = parcelHasValidLandCover.execute(
      createApplication(
        [
          { landCoverClassCode: '110', landCoverCode: '111' },
          { landCoverClassCode: '130', landCoverCode: '131' }
        ],
        [
          { landCoverClassCode: '110', areaSqm: 100 },
          { landCoverClassCode: '131', areaSqm: 50 }
        ]
      ),
      createRule()
    )
  ...
  test('should fail when the parcel matches neither the class code nor the cover code', () => {
    const result = parcelHasValidLandCover.execute(
      createApplication(
        [{ landCoverClassCode: '110', landCoverCode: '111' }],
        [{ landCoverClassCode: '130', areaSqm: 100 }]
  ...
  test('should fail when the land cover code matches but the area is zero', () => {
    const result = parcelHasValidLandCover.execute(
      createApplication(
        [{ landCoverCode: '111' }],
        [{ landCoverClassCode: '111', areaSqm: 0 }]
      ),
      createRule()
    )
  ...
  test('should fail when the action land cover code only matches a parcel cover with no area', () => {
    const result = parcelHasValidLandCover.execute(
      createApplication(
        [{ landCoverCode: '111' }],
        [{ landCoverClassCode: '111' }]
      ),
      createRule()
    )
