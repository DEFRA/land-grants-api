...
const runStatsQuery = async (client) => {
  const result = await client.query(`
    SELECT
      ...
      (SELECT COUNT(DISTINCT (sheet_id, parcel_id)) FROM land_parcels) AS "uniqueParcelsCount",
      (SELECT COUNT(DISTINCT (sheet_id, parcel_id)) FROM land_covers) AS "uniqueCoversCount",
