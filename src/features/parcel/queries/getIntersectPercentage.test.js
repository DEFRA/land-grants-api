...
const params = (sheetId, parcelId) => ({
  sheetId,
  parcelId,
  refCodes: ['X', 'Y'],
  dataLayerType: DATA_LAYER_TYPES.less_favoured_areas,
  operationName: 'Get test intersect percentage'
})

describe('getIntersectPercentage', () => {
  let mockDb
  let mockLogger
  let mockClient
  let mockResult
  ...
  test('should connect to the database', async () => {
    ...
    await getIntersectPercentage(params(sheetId, parcelId), mockDb, mockLogger)
  ...
  test('should return the overlap percentage', async () => {
    ...
    const result = await getIntersectPercentage(
      params(sheetId, parcelId),
      mockDb,
      mockLogger
    )
  ...
  test('should handle errors and return null', async () => {
    ...
    expect(mockLogger.error).toHaveBeenCalledWith(
      ...
      expect.stringContaining(
        'Database operation failed: Get test intersect percentage'
      )
    )
    expect(mockClient.release).toHaveBeenCalledTimes(1)
  })
