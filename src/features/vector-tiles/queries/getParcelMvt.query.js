...
-- ST_AsMVT is an aggregate over zero rows here, so an empty tile comes back as
-- no rows at all rather than a NULL tile; the caller maps both to an empty buffer.
SELECT ST_AsMVT(mvtgeom.*, 'parcels', ${MVT_EXTENT}, 'geom') AS tile
FROM mvtgeom
