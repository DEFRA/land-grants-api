...
import { paymentCalculateWMPSchemaV2, paymentCalculateWMPResponseSchemaV2 } from '../../schema/2.0.0/payment-calculate-wmp.controller.js'
...
export const PaymentsCalculateWMPControllerV2 = {
    options: {
        ...
        notes:
            'Calculates payment amounts for WMP',
        validate: {
            payload: paymentCalculateWMPSchemaV2,
            failAction: (_request, _h, err) => { throw Boom.badRequest('Invalid request payload input') }
        },
        response: {
            status: {
    ...
    */
    handler: async (request, h) => {
        ...
        const { parcelIds, areaHectares } = request.payload
        const parcels = parcelIds.map((parcelId) => splitParcelId(parcelId, request.logger))

        // get all parcels passed in
        const area = [];
        for (const parcel of parcels) {
            const landParcel = await getLandData(parcel.sheetId, parcel.parcelId, postgresDb, request.logger)
