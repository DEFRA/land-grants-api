  ...
  test('should import land covers data and return 200 ok', async () => {
    ...
    for (const fixture of fixtures) {
      const [coverResult] = await getRecordsByQuery(
        connection,
        'SELECT id, ST_AsText(c.geom) as geom, c.sheet_id, c.parcel_id, c.land_cover_class_code, c.is_linear_feature, c.last_updated, c.ingestion_date FROM land_covers c where c.id = $1',
        [fixture.ID]
      )
      ...
      expect(coverResult.ingestion_date).toBeDefined()
    }
